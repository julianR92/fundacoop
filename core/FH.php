...
class FH {
  ...
  public static function checkboxBlock($label,$name,$checked=false,$inputAttrs=[],$divAttrs=[],$errors=[]){
    ...
    $id = str_replace('[]','',$name);
    $html = '<div'.$divString.'>';
    $html .= ' <input type="checkbox" id="'.$id.'" name="'.$name.'" '.$checkString.$inputString.'>'.'<label for="'.$id.'" class="control-label">'.$label;
    $html .= '<span class="invalid-feedback">'.self::errorMsg($errors,$name).'</span>';
    $html .= '</label>';
    $html .= '</div>';
  ...
  public static function textareaBlock($label,$name,$value,$inputAttrs=[],$divAttrs=[],$errors=[]){
    ...
    $html .= '<textarea id="'.$id.'" name="'.$name.'"'.$inputString.' rows="4">'.$value.'</textarea>';
    $html .= '<span class="invalid-feedback">'.self::errorMsg($errors,$name).'</span>';
    $html .= '</div>';
    return $html;
  }
